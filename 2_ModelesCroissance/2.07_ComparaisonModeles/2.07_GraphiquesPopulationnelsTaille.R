...
# Graphique populationnel de tous les modeles taille
...
prediction <- data.frame(new_data, 
                         predictionJenssBayleyTaillePop = predict(JenssBayleyTaillePop, new_data),
                         predictionJenssModifieTaillePop = predict(JenssModifieTaillePop, new_data),
                         predictionReed1erOrdreTaillePop = predict(Reed1erOrdreTaillePop, new_data_no_zero),
                         predictionReed2ndOrdreTaillePop = predict(Reed2ndOrdreTaillePop, new_data_no_zero)
)

plot_populationnel <- ggplot(croiss, aes(x = age_an, y = taille)) + 
  theme_minimal() +
  geom_point(colour = "#999997", pch = 1) +
  geom_line(data = prediction,
            aes(x = age_an, y = predictionReed2ndOrdreTaillePop, 
                colour = "R2",
                linetype = "R2"),
            size = 1.5) +
  geom_line(data = prediction,
            aes(x = age_an, y = predictionReed1erOrdreTaillePop, 
                colour = "R1",
                linetype = "R1"),
            size = 1.5) +
  geom_line(data = prediction, 
            aes(x = age_an, y = predictionJenssBayleyTaillePop, 
                colour = "JB",
                linetype = "JB"),
            size = 1.5) +
  geom_line(data = prediction,
            aes(x = age_an, y = predictionJenssModifieTaillePop, 
                colour = "JA",
                linetype = "JA"),
            size = 1.5) +
  ...
  labs(
    title = "Modélisation populationnelle des effets fixes de quatre modèles structuraux pour la taille",
    x = "Âge (années)",
    y = "Taille (cm)",
...
pdf("Graphiques/Taille/GraphiquePopulationnelTaille.pdf", width = 12, height = 10)
plot_populationnel
dev.off()
