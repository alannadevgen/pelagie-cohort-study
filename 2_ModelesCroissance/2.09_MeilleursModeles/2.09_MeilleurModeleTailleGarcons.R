...
# Graphiques diagnostiques pour le meilleur modèle taille : Jenss adapte
...
# selectionner les donnees garcons
croiss_modele <- croiss_taille %>% filter(sexe == "M")
best.fit <- JenssModifieTailleGarcons
...
pdf("Graphiques/Taille/PredictionsIndividuellesJenssModifieTailleGarcons.pdf", paper = "a4r", width = 11, height = 10)
...
pdf("Graphiques/Taille/ScatterResidualsJenssModifieTailleGarcons.pdf", width = 12, height = 10)
ggplot(data = residus) +
  theme_minimal() +
  ...
  geom_hline(yintercept = -4, colour = "#941757", size = 0.9, linetype = "dashed") +
  labs(title = "Résidus du modèle Jenss adapté pour les garcons",
       x = "Prédiction individuelle",
