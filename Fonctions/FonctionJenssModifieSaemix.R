saemixJenssModifie <- function(
  ...
  } else if (covariable == FALSE){
    covariate.model <- matrix(0, ncol = nb_param, byrow = TRUE)
  }
  
  model <- saemixModel(
    model = modSaemixJenssModifie,
    description = paste0("Modele Jenss modifié - ", str_to_title(variable_anthropometrique)),
    psi0 = matrix(coefficients, ncol = nb_param, byrow = TRUE,
                  dimnames = list(NULL, c("Ap","Bp","Cp","Dp", "Ep"))),
  ...
  saemix.fit <- saemix(
    ...
    control = list(seed = 632545,
                   ...
                   directory = paste0("Graphiques/", str_to_title(variable_anthropometrique), "/GraphiquesJenssModifie", str_to_title(sexe))
                   )
  )
